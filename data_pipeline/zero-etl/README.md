...
* Run ```terraform init``` if required 
* Run ```terraform apply```
* After resources have deployed, can create the Zero ETL integration via the console (or similar)
* Then populate the source db with sample data. An easy way of doing this is connecting via a SQL client from an EC2 launched within the same demo VPC, with the self referencing security group associated with it. In the demo code, the database is publically accessible to make connecting easier, you just need to add a security group rule to allow access from your client
